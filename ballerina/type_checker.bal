...
# Represents the TypeChecker which is used by the runtime to check whether a type is a subtype of `rabbitmq:AnydataMessage`
public isolated class TypeChecker {
    ...
    isolated function isAnydataMessage(typedesc<anydata> typeDesc) returns boolean {
        if typeDesc is typedesc<AnydataMessage> {
            return true;
        }
        return false;
    }
